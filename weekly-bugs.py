...
# Constants: Statuses
STATUSES_ALL = ['NEW', 'ASSIGNED', 'MODIFIED', 'ON_DEV', 'ON_QA',
...
def get_cookies():
    ...
    paths = [
        ...
        os.path.join(home, 'Library/Application Support/Google/'
    ...
    # Query
    while True:
        cmd = ("sqlite3 -separator $'\t' \"%s\" "
               "'select host_key, name, value from cookies' "
               "| grep bugzilla.redhat.com" % (cookies_path))
...
def GET(url):
    ...
    # Fetch page content
    cmd = ('wget -O - -q --no-cookies '
           '--header="Cookie: %s" "%s"' % (cookies, url))
...
def URL_get_macro_devels(emails_ids, bug_statuses):
    return BUGLIST_URL + '&'.join(
        ...
        ['v%s=%s%%40redhat.com' % (emails_ids.index(e) + 1, e)
            for e in emails_ids])
...
def report_bugs_summary(project):
    ...
    l = [s.replace('RELEASE_PENDING', 'RELS_PEND').replace('MODIFIED', 'MOD')
         for s in STATUSES_ALL]
    table = prettytable.PrettyTable(l)
...
def main():
    ...
    parser.add_argument('--days', action="store", default=DEFAULT_DAYS,
                        help=("Number of days to cover (Default: %s)"
                              % (DEFAULT_DAYS)),
                        type=int)
    parser.add_argument('--untriaged-all', action="store_true", default=False,
                        help=("Include all untriaged bugs with an owner "
                              "(Default: No)"))
    ...
    if not team_bz:
        print ("ERROR: A 'bugzilla' block is required inside the %s's block. "
               "Please, check weekly_config.py" % (ns.team))
        raise SystemExit
