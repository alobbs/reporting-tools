...
def filter_data_from_time (data, from_time):
    return [l for l in data if 'subject' in l.keys() and l['lastUpdated'] >= from_time]

def render_owners (data, from_time):
    table = prettytable.PrettyTable (['Dev', 'Subject', 'ID', 'Date'], sortby='Date', sort_key=sort_rows, reversesort=True)
    for line in data:
        table.add_row ([line['owner']['username'], format_subject(line['subject']), line['number'], format_date(line['lastUpdated'])])
    return table.get_string()

def render_reviewers (data, from_time):
    table = prettytable.PrettyTable (['Reviewer', 'Subject', 'ID', 'Date'], sortby='Date', sort_key=sort_rows, reversesort=True)
    for line in data:
        table.add_row ([line['searched_for'], format_subject(line['subject']), line['number'], format_date(line['lastUpdated'])])
    return table.get_string()
...
def run_query (search_field, project, people, user, server, port):
    ...
    for nick in people:
        # Query
        query = "%(search_field)s:%(nick)s@redhat.com AND project:openstack/%(project)s" %(locals()) #  AND age:%(AGE)s

        # Excute
        cmd = "ssh %(user)s@%(server)s -p %(port)s gerrit query --format=JSON \"%(query)s\"" %(locals())
        f = os.popen (cmd, 'r')
        cont = f.read()
...
def main():
    ...
    for project in gerrit['projects']:
        for q_type in queries:
            data = run_query (q_type, project, gerrit['people'], user, server, port)
